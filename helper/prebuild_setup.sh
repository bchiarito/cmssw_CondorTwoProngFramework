...
if [[ "$1" == "sl7" ]]; then
  ...
  cmssw-el7 "--bind /condor --bind /osg --bind /cms --bind /home --bind /users" -- helper/prebuild.sh $2 $4
  elif [[ "$3" == "cmslpc" ]]; then
  cmssw-el7 "--bind /uscms_data/d1/$USER/" -- helper/prebuild.sh $2 $4
  elif [[ "$3" == "lxplus" ]]; then
  cmssw-el7 -- helper/prebuild.sh $2 $4
  fi
...
cmssw-el7 -- helper/test_payload.sh
fi
